/*
 * Tray ESP32 Firmware
 * ===================
 * Drives the two tray stepper motors. Talks to the micro-ROS ESP32 over
 * ESP-NOW peer-to-peer radio (no wires between the boards).
 *
 * Updated:
 * - Added separate ENABLE pins for both stepper drivers
 * - Motor driver is enabled only when that tray is moving
 * - ESP-NOW callback signatures fixed for newer ESP32 core
 * - Ultrasonic checked only during OPEN_DWELL
 * - If ultrasonic distance > 37 cm continuously for 5 seconds, tray closes
 * - Added second ultrasonic sensor for tray 2
 *
 * ENABLE assumption:
 *   LOW  = enabled
 *   HIGH = disabled
 */

#include <Arduino.h>
#include <AccelStepper.h>
#include <WiFi.h>
#include <esp_now.h>
#include <esp_wifi.h>

// =========================
// Motor 1 pins
// =========================
#define M1_STEP_PIN 33
#define M1_DIR_PIN  32
#define M1_M0_PIN   27
#define M1_M1_PIN   26
#define M1_M2_PIN   25
#define M1_EN_PIN   14

// =========================
// Motor 2 pins
// =========================
#define M2_STEP_PIN 19
#define M2_DIR_PIN  18
#define M2_M0_PIN   23
#define M2_M1_PIN   22
#define M2_M2_PIN   21
#define M2_EN_PIN   13

// =========================
// Ultrasonic pins - Tray 1
// =========================
#define TRAY1_TRIG_PIN 2
#define TRAY1_ECHO_PIN 4

// =========================
// Ultrasonic pins - Tray 2
// =========================
#define TRAY2_TRIG_PIN 16
#define TRAY2_ECHO_PIN 17

const float CLOSE_DISTANCE_CM = 37.0;
const unsigned long DISTANCE_CONFIRM_MS = 5000;   // wait 5 sec
const unsigned long ULTRA_CHECK_INTERVAL_MS = 80; // check only during dwell

// =========================
// ESP-NOW link
// =========================
static uint8_t MICRO_ROS_ESP_MAC[6] = { 0xF4, 0x2D, 0xC9, 0x70, 0x9F, 0x00 };

#define ESPNOW_CHANNEL       1
#define ESPNOW_MAX_RETRIES   3
#define ESPNOW_RETRY_GAP_MS  50

#define MSG_TRAY_OPEN_CMD    1
#define MSG_TRAY_CLOSED_ACK  2

typedef struct __attribute__((packed)) {
  uint8_t  msg_type;
  uint8_t  tray_id;
  uint32_t seq;
} TrayPacket;

// =========================
// Motion profile
// =========================
const long FORWARD_STEPS  = 10000;
const long BACKWARD_STEPS = 9000;
const float STEPPER_MAX_SPEED    = 2000.0f;
const float STEPPER_ACCELERATION = 800.0f;

// =========================
// Timing
// =========================
const unsigned long TRAY_OPEN_DWELL_MS = 30000;

// If true, motor driver is disabled while tray is open and waiting.
// If tray slips while open, change this to false.
const bool DISABLE_DURING_DWELL = true;

// =========================
// Stepper objects
// =========================
AccelStepper stepper1(AccelStepper::DRIVER, M1_STEP_PIN, M1_DIR_PIN);
AccelStepper stepper2(AccelStepper::DRIVER, M2_STEP_PIN, M2_DIR_PIN);

// =========================
// State machine
// =========================
enum TrayState {
  IDLE,
  OPENING,
  OPEN_DWELL,
  CLOSING
};

TrayState state = IDLE;
int active_tray = 0;
unsigned long state_entered_ms = 0;
unsigned long last_ultra_check_ms = 0;
unsigned long distance_above_37_since = 0;

// Pending command from ESP-NOW callback
volatile int pending_tray_cmd = 0;
volatile uint32_t last_rx_cmd_seq = 0;

// TX bookkeeping
volatile bool espnow_last_send_ok = false;
volatile bool espnow_send_done    = false;
uint32_t tx_seq = 0;

// =========================
// Enable helpers
// =========================
void setMotorEnabled(uint8_t enPin, bool enable) {
  digitalWrite(enPin, enable ? LOW : HIGH);
}

void enableOnlyMotor(int tray) {
  if (tray == 1) {
    setMotorEnabled(M1_EN_PIN, true);
    setMotorEnabled(M2_EN_PIN, false);
  } else if (tray == 2) {
    setMotorEnabled(M1_EN_PIN, false);
    setMotorEnabled(M2_EN_PIN, true);
  } else {
    setMotorEnabled(M1_EN_PIN, false);
    setMotorEnabled(M2_EN_PIN, false);
  }
}

void disableAllMotors() {
  setMotorEnabled(M1_EN_PIN, false);
  setMotorEnabled(M2_EN_PIN, false);
}

// =========================
// Helpers
// =========================
void set_microstep_half(int m0, int m1, int m2) {
  digitalWrite(m0, HIGH);
  digitalWrite(m1, LOW);
  digitalWrite(m2, LOW);
}

AccelStepper& motor_for(int tray) {
  return (tray == 1) ? stepper1 : stepper2;
}

void enter_state(TrayState s) {
  state = s;
  state_entered_ms = millis();
}

// =========================
// Ultrasonic helpers
// =========================
void ultrasonicPinsForTray(int tray, int &trigPin, int &echoPin) {
  if (tray == 1) {
    trigPin = TRAY1_TRIG_PIN;
    echoPin = TRAY1_ECHO_PIN;
  } else {
    trigPin = TRAY2_TRIG_PIN;
    echoPin = TRAY2_ECHO_PIN;
  }
}

float readUltrasonicDistanceCm(int tray) {
  int trigPin, echoPin;
  ultrasonicPinsForTray(tray, trigPin, echoPin);

  digitalWrite(trigPin, LOW);
  delayMicroseconds(2);

  digitalWrite(trigPin, HIGH);
  delayMicroseconds(10);
  digitalWrite(trigPin, LOW);

  long duration = pulseIn(echoPin, HIGH, 12000);

  if (duration == 0) {
    return -1.0;
  }

  float distance = duration * 0.0343f / 2.0f;
  return distance;
}

// =========================
// ESP-NOW callbacks
// =========================
void espnow_on_send(const esp_now_send_info_t *tx_info, esp_now_send_status_t status) {
  (void)tx_info;
  espnow_last_send_ok = (status == ESP_NOW_SEND_SUCCESS);
  espnow_send_done    = true;
}

void espnow_on_recv(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
  (void)info;

  if (len != (int)sizeof(TrayPacket)) return;

  TrayPacket pkt;
  memcpy(&pkt, data, sizeof(pkt));

  if (pkt.msg_type != MSG_TRAY_OPEN_CMD) return;
  if (pkt.tray_id != 1 && pkt.tray_id != 2) return;

  if (pkt.seq != 0 && pkt.seq == last_rx_cmd_seq) return;
  last_rx_cmd_seq = pkt.seq;

  pending_tray_cmd = pkt.tray_id;
}

bool espnow_send_with_retry(const TrayPacket& pkt) {
  for (int attempt = 0; attempt < ESPNOW_MAX_RETRIES; attempt++) {
    espnow_send_done    = false;
    espnow_last_send_ok = false;

    esp_err_t rc = esp_now_send(MICRO_ROS_ESP_MAC, (const uint8_t*)&pkt, sizeof(pkt));
    if (rc == ESP_OK) {
      unsigned long t0 = millis();
      while (!espnow_send_done && (millis() - t0) < 50) {
        delay(1);
      }
      if (espnow_last_send_ok) return true;
    }

    delay(ESPNOW_RETRY_GAP_MS);
  }
  return false;
}

void setup_espnow() {
  WiFi.mode(WIFI_STA);
  esp_wifi_set_channel(ESPNOW_CHANNEL, WIFI_SECOND_CHAN_NONE);

  if (esp_now_init() != ESP_OK) {
    Serial.println("ESP-NOW init failed");
    return;
  }

  esp_now_register_send_cb(espnow_on_send);
  esp_now_register_recv_cb(espnow_on_recv);

  esp_now_peer_info_t peer = {};
  memcpy(peer.peer_addr, MICRO_ROS_ESP_MAC, 6);
  peer.channel = ESPNOW_CHANNEL;
  peer.encrypt = false;

  if (!esp_now_is_peer_exist(MICRO_ROS_ESP_MAC)) {
    if (esp_now_add_peer(&peer) != ESP_OK) {
      Serial.println("ESP-NOW add_peer failed");
      return;
    }
  }

  Serial.println("ESP-NOW initialized, peer added.");
}

// =========================
// Setup / loop
// =========================
void setup() {
  Serial.begin(115200);
  delay(100);

  pinMode(M1_M0_PIN, OUTPUT);
  pinMode(M1_M1_PIN, OUTPUT);
  pinMode(M1_M2_PIN, OUTPUT);

  pinMode(M2_M0_PIN, OUTPUT);
  pinMode(M2_M1_PIN, OUTPUT);
  pinMode(M2_M2_PIN, OUTPUT);

  pinMode(M1_EN_PIN, OUTPUT);
  pinMode(M2_EN_PIN, OUTPUT);
  disableAllMotors();

  pinMode(TRAY1_TRIG_PIN, OUTPUT);
  pinMode(TRAY1_ECHO_PIN, INPUT);
  digitalWrite(TRAY1_TRIG_PIN, LOW);

  pinMode(TRAY2_TRIG_PIN, OUTPUT);
  pinMode(TRAY2_ECHO_PIN, INPUT);
  digitalWrite(TRAY2_TRIG_PIN, LOW);

  set_microstep_half(M1_M0_PIN, M1_M1_PIN, M1_M2_PIN);
  set_microstep_half(M2_M0_PIN, M2_M1_PIN, M2_M2_PIN);

  stepper1.setMaxSpeed(STEPPER_MAX_SPEED);
  stepper1.setAcceleration(STEPPER_ACCELERATION);

  stepper2.setMaxSpeed(STEPPER_MAX_SPEED);
  stepper2.setAcceleration(STEPPER_ACCELERATION);

  setup_espnow();

  enter_state(IDLE);
  Serial.println("Tray ESP32 ready. Waiting for ESP-NOW tray command...");
}

void loop() {
  switch (state) {

    case IDLE: {
      int cmd = pending_tray_cmd;

      if (cmd == 1 || cmd == 2) {
        pending_tray_cmd = 0;
        active_tray = cmd;

        Serial.print("OPEN_CMD received: opening tray ");
        Serial.println(active_tray);

        enableOnlyMotor(active_tray);

        AccelStepper& m = motor_for(active_tray);
        m.setCurrentPosition(0);
        m.move(FORWARD_STEPS);

        enter_state(OPENING);
      }
      break;
    }

    case OPENING: {
      AccelStepper& m = motor_for(active_tray);
      m.run();

      if (m.distanceToGo() == 0) {
        Serial.print("Tray ");
        Serial.print(active_tray);
        Serial.println(" open. Dwelling...");

        if (DISABLE_DURING_DWELL) {
          disableAllMotors();
        }

        last_ultra_check_ms = 0;
        distance_above_37_since = 0;
        enter_state(OPEN_DWELL);
      }
      break;
    }

    case OPEN_DWELL: {
      bool timeExpired = (millis() - state_entered_ms >= TRAY_OPEN_DWELL_MS);
      bool closeByDistance = false;

      if (millis() - last_ultra_check_ms >= ULTRA_CHECK_INTERVAL_MS) {
        last_ultra_check_ms = millis();

        float distance = readUltrasonicDistanceCm(active_tray);

        Serial.print("Tray ");
        Serial.print(active_tray);
        Serial.print(" Ultrasonic: ");

        if (distance < 0) {
          Serial.println("No reading");
          distance_above_37_since = 0;
        } else {
          Serial.print(distance);
          Serial.println(" cm");

          if (distance > CLOSE_DISTANCE_CM) {
            if (distance_above_37_since == 0) {
              distance_above_37_since = millis();
              Serial.print("Tray ");
              Serial.print(active_tray);
              Serial.println(" distance > 37 cm detected. Starting 5-second wait...");
            } else if (millis() - distance_above_37_since >= DISTANCE_CONFIRM_MS) {
              closeByDistance = true;
              Serial.print("Tray ");
              Serial.print(active_tray);
              Serial.println(" distance > 37 cm for 5 seconds. Closing tray...");
            }
          } else {
            distance_above_37_since = 0;
          }
        }
      }

      if (timeExpired || closeByDistance) {
        enableOnlyMotor(active_tray);

        AccelStepper& m = motor_for(active_tray);
        m.move(-BACKWARD_STEPS);

        Serial.print("Closing tray ");
        Serial.println(active_tray);

        enter_state(CLOSING);
      }
      break;
    }

    case CLOSING: {
      AccelStepper& m = motor_for(active_tray);
      m.run();

      if (m.distanceToGo() == 0) {
        Serial.print("Tray ");
        Serial.print(active_tray);
        Serial.println(" closed. Sending CLOSED_ACK...");

        disableAllMotors();

        TrayPacket pkt;
        pkt.msg_type = MSG_TRAY_CLOSED_ACK;
        pkt.tray_id  = (uint8_t)active_tray;
        pkt.seq      = ++tx_seq;

        bool ok = espnow_send_with_retry(pkt);
        Serial.println(ok ? "CLOSED_ACK delivered." : "CLOSED_ACK send failed after retries.");

        active_tray = 0;
        enter_state(IDLE);
      }
      break;
    }
  }
}