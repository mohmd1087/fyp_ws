...
class DStarLite:
    ...
    def c(self, u: (int, int), v: (int, int)) -> float:
        if self.sensed_map is None:
            print("⚠️ sensed_map is None in cost function — skipping this edge")
            return float('inf')  # fallback to very high cost
            
        try:
            if not self.sensed_map.is_unoccupied(u) or not self.sensed_map.is_unoccupied(v):
                return float('inf')
        except Exception as e:
            print(f"💥 Error in c(): {e} — u: {u}, v: {v}")
            return float('inf')

        base_cost = heuristic(u, v)

    # 👇 NEW: Penalize cells close to obstacles
        danger_radius = 8  # in cells (e.g., 2-cell radius)
        penalty = 0

        for dx in range(-danger_radius, danger_radius + 1):
            for dy in range(-danger_radius, danger_radius + 1):
                nx, ny = v[0] + dx, v[1] + dy
                if 0 <= nx < self.sensed_map.x_dim and 0 <= ny < self.sensed_map.y_dim:
                    if not self.sensed_map.is_unoccupied((nx, ny)):
                        penalty += 1
                        
        penalty = min(penalty, 20)     # cap it
        return base_cost + penalty * 4  # ⬅️ Tune weight as needed
    ...
    def move_and_replan(self, robot_position: (int, int)):
        if robot_position is None:
            raise ValueError("🚨 move_and_replan called with None robot_position")

        if not (0 <= robot_position[0] < self.sensed_map.x_dim and 0 <= robot_position[1] < self.sensed_map.y_dim):
            raise ValueError(f"🚨 robot_position {robot_position} out of bounds")
        path = [robot_position]
