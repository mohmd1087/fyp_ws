from fyp_dstar_lite.priority_queue import PriorityQueue, Priority
from fyp_dstar_lite.grid import OccupancyGridMap
import numpy as np
from fyp_dstar_lite.utils import heuristic, Vertex, Vertices
...
class DStarLite:
    ...
    def move_and_replan(self, robot_position: (int, int)):
        ...
        if not (0 <= robot_position[0] < self.sensed_map.x_dim and
                0 <= robot_position[1] < self.sensed_map.y_dim):
        ...
        max_steps = 100000  # safety guard against infinite loop
        steps = 0

        while self.s_start != self.s_goal:
            steps += 1
            if steps > max_steps:
                raise ValueError(f"move_and_replan exceeded {max_steps} steps — possible loop")
            ...
            if arg_min is None:
                raise ValueError("No reachable successor from current position — robot trapped")
